...
/* Dual-mode fuzzing harness for flux_sign_unwrap() parser.
 * Supports both AFL++ persistent mode and libFuzzer.
 *
 * Uses FLUX_SIGN_NOVERIFY to test parsing without crypto overhead.
 * This is faster and focuses on parser bugs.
 *
 ...
 * Tests: header_decode, payload_decode_cpy, kv_decode,
 * base64 decoding, and format validation.
 */
static int fuzz_sign_unwrap_noverify(flux_security_t *ctx,
                                      const uint8_t *data,
                                      size_t size)
{
    const void *payload;
    ...
    /* NOVERIFY: skip signature verification, test parser only */
...
int main (void)
{
    ...
    while (__AFL_LOOP (10000)) {
        ...
        fuzz_sign_unwrap_noverify (ctx, buf, len);
    }
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    return fuzz_sign_unwrap_noverify (global_ctx, data, size);
}
