...
/* Dual-mode fuzzing harness for kv_decode() parser.
 ...
 *   Where T is a type hint character: s=string, i=int64, d=double, b=bool, t=timestamp
 *
 ...
 */
static int fuzz_kv(const uint8_t *data, size_t size)
{
    struct kv *kv;
    ...
    /* Attempt to decode KV structure */
    kv = kv_decode ((const char *)data, size);
